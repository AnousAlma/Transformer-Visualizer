...
export default function SelfAttentionScreen({ stepIndex, setStepIndex, inputText, layer, head, setHead }: {
  stepIndex: number; setStepIndex: (n: number) => void; inputText: string; layer: number; head: number; setHead: (h: number) => void
}) {
  // ✅ Fixed: was incorrectly using "embedding" namespace
    ...
    <div className="grid grid-cols-[2fr_1fr] gap-10">
      ...
      <div className="w-full shrink-0 bg-[#0e0e11] border border-[#1e1e24] rounded-2xl p-5 flex flex-col gap-5">
        ...
        {!loading && tokens.length > 0 && attentionMatrix[queryToken] && (
          <div className="border border-[#1e1e24] rounded-xl p-3 flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1.5 text-xs font-mono">
              ...
              <div className="flex justify-between"><span className="text-zinc-600">{t("topAttended")}</span><span className="text-purple-300">"{tokens[attentionMatrix[queryToken].indexOf(Math.max(...attentionMatrix[queryToken]))]}"</span></div>
        ...
        <div className="mt-auto flex justify-end">
          <button onClick={() => setStepIndex(stepIndex + 1)} className="px-4 py-2 rounded-lg text-xs border border-[#2a2a2e] text-zinc-400 hover:bg-[#1a1a20] hover:text-zinc-200 transition">
            {t("next")}
          </button>
        </div>
      </div>
