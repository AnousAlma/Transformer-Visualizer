  ...
  return (
    <div ref={canvasRef} className="w-full">
      ...
      <svg viewBox="0 0 520 200" width="100%" style={{ display: "block" }}>
        ...
        {vecDims.map((d, i) => (
          <circle key={i} cx={10 + 10 + (i % 8) * 12} cy={60 + 16 + Math.floor(i / 8) * 18}
            r="4.5" fill={d.color} opacity={d.alpha} />
        ))}
        ...
        <rect x="180" y="40" width="110" height="120" rx="10"
          ...
          className="box-pulse" />
        {linDims.map((d, i) => (
          <circle key={i} cx={180 + 16 + (i % 6) * 15} cy={40 + 20 + Math.floor(i / 6) * 22}
            r="4" fill="#6366f1" className="dim-light"
        ...
        <rect x="350" y="60" width="90" height="80" rx="10"
          ...
          className="box-pulse" />
        {smDims.map((d, i) => (
          <circle key={i} cx={350 + 12 + (i % 5) * 14} cy={60 + 18 + Math.floor(i / 5) * 20}
            r="4" fill="#06b6d4" className="dim-light"
            style={{ "--base-alpha": d.alpha, "--anim-delay": `${d.delay}ms` } as React.CSSProperties}
            opacity={d.alpha} />
        ))}
...
export default function ProbabilityScreen({
  ...
  const [tokens, setTokens] = useState<string[]>(["The"])
    ...
    <div className="grid grid-cols-[2fr_1fr] gap-10">
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-5 mt-2">
          {[
            {
              num: "1", color: "rgba(168,85,247,0.4)", textColor: "text-purple-400",
              label: "Final hidden state · 768 dims",
              desc: "The full representation of this token after all attention and MLP layers.",
            },
            {
              num: "2", color: "rgba(99,102,241,0.4)", textColor: "text-indigo-400",
            ...
            {
              num: "3", color: "rgba(6,182,212,0.4)", textColor: "text-cyan-400",
